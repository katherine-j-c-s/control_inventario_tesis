...
const getAllProjects = async (req, res) => {
    try {
        ...
        const projects = await projectRepository.find({
            select: ['project_id', 'admin_id', 'name', 'description', 'ubicacion', 'created_at', 'updated_at']
        });
        res.json(projects);
    } catch (error) {
...
const createProject = async (req, res) => {
    try {
        const { admin_id, name, description, ubicacion } = req.body;
        const projectRepository = AppDataSource.getRepository('Project');

        const project = projectRepository.create({ admin_id, name, description, ubicacion });
